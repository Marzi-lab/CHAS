...
#'
#' @details
#' This function takes two inputs, bulk peak location and reference
#' peak location
#'
...
#' @param refPeakList A list containing several data frames,
#' each data frame contains the reference
...
#'   for example, in MacOS, this can be checked by running
#'   "percent which bedtools" in Terminal
#' @import stringi
#' @import tidyverse
#' @import tidyr
#' @import readr
...
UnionPeaks <- function(bulkPeaks,refPeakList,bedtools_path){
  ...
  utils::write.table(merPeaks, 'merPeaks.bed',
                     row.names = FALSE,
                     col.names = FALSE,
                     sep = '\t',
                     quote=FALSE)
  ...
  system(paste(bedtools_path,
               "merge -i merPeaks.bed -c 4 -o distinct -d 20 > unionPeaks.bed"))
  unionPeaks <- utils::read.table("unionPeaks.bed")
  ...
  celltypePeaks <- celltypePeaks[celltypePeaks$celltype==1 &
                                 celltypePeaks$bulk==TRUE,]
