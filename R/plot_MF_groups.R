...
plot_MF_groups <- function(celltypeProportion, group){
  ...
  # plotting
  plot <- ggplot(MF_long, aes(fill=celltype, y=Score, x=celltype, pattern = Group, pattern_type = Group)) +
    ...
    labs(x = "Cell type", y = "MF score") + ylim(min(MF_long$Score)-0.05,max(MF_long$Score)+0.05) +
