...
plot_celltype_props <- function(annotatedPeaks, exampleData=TRUE){
  ...
  if (exampleData == TRUE) {
    PropTable$Var1 <- factor(PropTable$Var1, levels = c("Astrocytes", "Microglia", "Neurons", "Oligodendrocytes", "Multiple", "Other"))
    plot <- ggplot(PropTable, aes(x=Source, y=Freq, fill=Var1)) + geom_bar(stat="identity") + theme_classic() + scale_fill_manual(values=c("#446455", "#FDD262", "#46ACC8", "#F4B5BD", "#E58601", "#B40F20")) + ylab("Proportion of peaks specific to each cell type (%)") + xlab("Peaks") + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) + guides(fill=guide_legend(title="Cell type"))
  } else {
    plot <- ggplot(PropTable, aes(x=Source, y=Freq, fill=Var1)) + geom_bar(stat="identity") + theme_classic() + scale_fill_brewer(palette="Accent") + ylab("Proportion of peaks specific to each cell type (%)") + xlab("Peaks") + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) + guides(fill=guide_legend(title="Cell type"))
  }
  return(plot)
}
...
plot_celltype_scores <- function(celltypeScores, pheno){
  ...
  annotScores <- merge(celltypeScores, AD_pheno, by="Sample")

  plot <- ggplot(annotScores, aes(Celltype, Score, fill=Group)) + geom_violin() + labs(x="Cell type", y="Cell type score") + theme_classic() + scale_fill_manual(values = wes_palette("Darjeeling2"))
