...
### Route A. With bam files for bulk and reference samples

**1. Create consensus peaks**

*CHAS-MF* merges bulk peaks and reference peaks to create consensus peaks. Each consensus peaks are then annotated with corresponding cell types, depending on whether the consensus peak covers cell type-specific signals. A SAF file containing the consensus peaks will be generated during this step, to be used for read counting.
...
### Route B. Without bam files for bulk and reference samples

**1. Create consensus peaks and use raw counts as approximate**

*CHAS-MF* merges bulk peaks and reference peaks to create consensus peaks. Each consensus peaks are then annotated with corresponding cell types, depending on whether the consensus peak covers cell type-specific signals. The original read counts for bulk and reference peaks will be used as an approximate for the read counts of each bulk and reference sample for the consensus peaks. If there are multiple peaks from one sample merging into the same consensus peak, the read count for the first peak will be used as an approximate for the total read count for the entire consensus region.
...
*UnionPeaks()* takes as input the bulk peaks, the list of cell sorted peaks, and the path to bedtools. It outputs a list of two data frames. The first data frame contains the locations of consensus peaks. The second data frame contains the consensus peaks that are found in both the bulk samples and at least one reference cell type (for each cell type, TRUE means present, FALSE means absent). Both data frames will be used in the following *CelltypeProportion()* function. *UnionPeaks()* also generates a BED file called 'unionPeaks.bed' which contains all the consensus peaks, and a SAF file called 'unionPeaks.saf' which will be used for read counting.

### 5. Read counting

Use whichever read counting method you prefer to count the reads from raw bam files for bulk and reference samples. Peak locations for the consensus peaks are in the SAF file generated in the previous step. The following is an example using featureCounts.

```{bash, eval=FALSE}
featureCounts -F SAF -O -T 8 -p -a unionPeaks.saf -o AD_unionCounts_bulk.txt ./bulk/*.bam
featureCounts -F SAF -O -T 8 -p -a unionPeaks.saf -o AD_unionCounts_ref.txt ./reference/*.bam
```

Import bulk and reference counts into your R environment:

```{r, eval=FALSE}
AD_unionCounts_bulk <- read.table("AD_unionCounts_bulk.txt", header = TRUE)
AD_unionCounts_ref <- read.table("AD_unionCounts_ref.txt", header = TRUE)
```

After loading the counts data, remember to edit the column ID for both data frames to contain only the sample names, not the entire path for the bam files. 

### 6. Predict cell type proportions

For the purpose of this tutorial, we have done the above read counting steps on our own reference and bulk bam files. The results can be loaded from:

```{r}
data(AD_unionCounts_bulk)
data(AD_unionCounts_ref)
```

First make a table containing two columns. The first column contains the sample ID used in the reference counts table (in this example, in the AD_unionCounts_ref table) and in the same order. The second column contains the corresponding cell type for each sample.

```{r}
refSamples <- data.frame(matrix(nrow=12,ncol=2))
refSamples[,1] <- names(AD_unionCounts_ref)
...
```{r}
AD_MF_routeA <- CelltypeProportion(AD_unionCounts_bulk, AD_unionCounts_ref, AD_unionPeaks$unionPeaks, refSamples, AD_unionPeaks$celltypePeaks)
```

*CelltypeProportion()* takes as input the bulk and reference counts for consensus peaks, the locations of the consensus peaks, cell type annotations for reference samples, and 'signature peaks', whcih are consensus peaks that are found in both the bulk and reference samples. This function first calculates length-normalised cpm (counts per million) from raw counts, then calculates the median and variability for reference data, then run matrix factorisation on the signature peaks using the R package EPIC. The output is a list containing two items: the first item is the number of signature peaks used, and the second item is a table containing the predicted proportions of each cell type in each bulk sample. The proportion of uncharacterised cell types is also predicted in the 'otherCells' column.

### 7. Plot the predicted cell type proportions in the bulk brain samples.

``` {r, fig.width=7, fig.height=3.5}
plot_MF_props(AD_MF_routeA)
```

*plot_MF_props()* uses the predicted cell type proportions from *CelltypeProportion()* to generate a stacked bar plot of the proportions of each cell type in each of the bulk brain H3K27ac sample.

### 8. Plot the predicted cell type proportions between two groups.

``` {r, fig.width=4.5, fig.height=3.5}
plot_MF_groups(AD_MF_routeA, AD_pheno)
```
...
plot_correlation(AD_MF_routeA, celltype_scores, AD_pheno)
```
...
<p> The cell sorted H3K27ac peaks (called on all purified cell samples merged together) are available in two human genome reference builds hg19 and hg38.<br> 
...
```{r}
refSamples <- data.frame(matrix(nrow=15,ncol=2))
refSamples[,1] <- names(ref_H3K27ac_counts_hg38)
refSamples[,2] <- sub(".*_", "", names(ref_H3K27ac_counts_hg38))
```

To predict cell type proportions, run the following function on bulk and reference counts for consensus peaks.

```{r}
AD_MF_routeB <- CelltypeProportion(AD_consensusPeaks$newBulkTPM, AD_consensusPeaks$newRefTPM, AD_consensusPeaks$consensusPeaks, 
                                   refSamples, NULL)
```

*CelltypeProportion()* takes as input the bulk and reference counts for consensus peaks, the locations of the consensus peaks, cell type annotations for reference samples. The 'signature' input is left as NULL, because in this MF route, *CelltypeProportion()* will automatically select the signature peaks based on reference counts - it only keeps the peaks whose maximum counts in all cell types is >= 5 times higher than the second largest. This ensures that the selected peaks have high signal in only one cell type, hence a 'signature' for that cell type. *CelltypeProportion()* then run matrix factorisation on the signature peaks using the R package EPIC. The output is a list containing two items: the first item is the number of signature peaks used, and the second item is a table containing the predicted proportions of each cell type in each bulk sample. The proportion of uncharacterised cell types is also predicted in the 'otherCells' column.

### 5. Plot the predicted cell type proportions in the bulk brain samples.

``` {r, fig.width=7, fig.height=3.5}
plot_MF_props(AD_MF_routeB)
```

*plot_MF_props()* uses the predicted cell type proportions from *CelltypeProportion()* to generate a stacked bar plot of the proportions of each cell type in each of the bulk brain H3K27ac sample.

### 6. Plot the predicted cell type proportions between two groups.

``` {r, fig.width=4.5, fig.height=3.5}
plot_MF_groups(AD_MF_routeB, AD_pheno)
```
...
``` {r, fig.width=6, fig.height=5}
plot_correlation(AD_MF_routeB, celltype_scores, AD_pheno)
```
